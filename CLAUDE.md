...
Every publish also pushes an immutable **`sha-<short7>`** tag (first 7 chars of the built commit) alongside the mutable `:stag`/`:prod` tags. Hub ECS deploys pin these sha tags via task-definition revisions — `make deploy-centralhub` resolves the env branch tip's tag, `make rollback-centralhub` repoints at the previous revision (FLIP#751; see `deploy/providers/AWS/README.md` "Central Hub deploys and rollback"). `deploy-centralhub` also runs an **FL quiesce pre-flight** (FLIP#770): replacing `fl-server-net-1` kills any in-flight training run, so the deploy refuses unless the hub's `GET /fl/quiesce` reports deployment mode ON and no BUSY net — deployment mode pauses FL job pickup (queued jobs hold; the running job finishes and frees its net), making "enable mode → wait → deploy → disable" the standard redeploy workflow.
