...
- `FL_KIT_SLOT_NAMES` — JSON list (e.g. `["Trust_1", "Trust_2"]`) of FL kit-slot names seeding the hub's `fl_kit_slot` pool that `register_trust` claims from; each name must match a provisioned participant kit (in-tree workspace for dev, `s3://<AICENTRE_BUCKET_NAME>/fl-flare-participant-kits/<FLARE_KIT_DATE>/net-<N>/services/<slot>/` for stag/prod). Read **only at flip-api boot** by `seed_fl_kit_slots` (additive — never deletes or re-assigns rows), so pool changes need a flip-api restart; an exhausted pool raises `NoFreeKitSlotError` on trust registration. In stag/prod the value is baked into the flip-api ECS task definition (`locals.tf` via `TF_VAR_FL_KIT_SLOT_NAMES`), so adding a slot needs a new task-definition revision + service update — `make deploy-centralhub` only swaps image tags and will not propagate env changes. Full runbook (mint kit → additive S3 upload → activate slot → register trust): `fl-services/nvflare/README.md` ("Onboarding a new client onto an existing network").
