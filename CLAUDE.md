...
- `DB_IAM_AUTH` — `false` by default. When `true`, flip-api authenticates to Postgres with a short-lived AWS IAM auth token minted per-connection (via RDS Proxy) by a SQLAlchemy `do_connect` hook in `flip_api/db/database.py`, instead of a static password baked into the engine URL at boot. Set to `true` in production by the ECS task env (`deploy/providers/AWS/locals.tf`); the static `POSTGRES_PASSWORD`/`POSTGRES_SECRET_ARN` path is the dev default and the prod rollback. Fixes the prod DB outage on every RDS secret rotation (FLIP#556) — the app no longer caches a credential that goes stale.
