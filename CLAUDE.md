...
- `MLFLOW_TRACKING_URI` — MLflow dual-write (FLIP#745): tracking server URI read by the fl-servers (the `flip` package's best-effort `MlflowSink` in `flip-utils/flip/core/mlflow_sink.py`), flip-api (`mlflow_run_service` pre-creates the run with tags + config.json params at job submit), and the NVFLARE tutorial simulator. **Empty (the default) disables the integration entirely** — flip-api stays canonical for status/UI metrics; MLflow is an additive mirror, never in the training critical path. Dev value `http://mlflow:5000`; the dev `mlflow` service (pinned `ghcr.io/mlflow/mlflow`, sqlite-on-volume) publishes its UI on `127.0.0.1:${MLFLOW_PORT:-5000}` only and joins `central-hub-network` + the hub-only `central-hub-mlflow-network` bridge — NEVER `shared-net-1/2` (trust fl-clients live there; MLflow has no auth, network placement is the boundary). Result zips are registered as model versions **by S3 reference URI** — weights are never copied into MLflow. Not deployed in prod (hosting decision deferred; see #745 WP6). See `docs/source/components/component-mlflow.rst`.
