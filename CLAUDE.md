...
- `PRE_SIGNED_URL_EXPIRATION_SECONDS` — Time-to-live for a model-file presigned POST policy, in seconds. Researchers have this long to start uploading after requesting the URL. Default `3600` (1 hour); no upper bound is enforced beyond AWS's own SigV4 7-day limit.
