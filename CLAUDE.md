...
- **FL apps never download at run time** (#1206). No `pretrained=True`, `torch.hub.load`, `from_pretrained("org/x")` or `load_state_dict_from_url` in anything under `fl-tutorials/**/app*/` or `fl-apps/`: the FL server on a platform-managed estate and a trust host behind an NHS firewall have no internet route (the ServerApp hangs, the client dies with `cannot sync with server Runner`, the net stays BUSY), and a run-time fetch bypasses the scanned upload path a reviewer approved — a swapped `.pth` is code execution beside patient data. Weights ship as uploaded files (`.safetensors` preferred); a tutorial that needs one stages it with its own `make weights` (shared download in `fl-tutorials/datasets/Makefile`, e.g. `download-ldm-weights`). `fl-tutorials/tests/test_offline_apps.py` is the AST guard; `fl-tutorials/datasets/` (host-side download tooling) is exempt by design.
