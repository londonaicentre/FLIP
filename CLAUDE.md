...
│   └── deploy/         # Per-env/per-backend trust compose matrix (compose_trust.{development,production}[.flower|.nvflare|.gpu].yml, etc.)
...
make deploy-trust-k8s      # Deploy trust services to Kubernetes via Helm (forwards to deploy/providers/kubernetes)
...
(`deploy/providers/local/**` Ansible). **UI/release/branch hygiene** —
