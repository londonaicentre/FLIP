...
// Cribs the flow from integration/group-2/model/delete_model.spec.ts —
// open the model edit drawer, click delete, confirm by typing the model name.
...
describe("docs: delete model", () => {
    it("deletes a model after confirming its name", () => {
        cy.login();
        ...
        cy.intercept("DELETE", `/model/${MODEL_ID}`, { statusCode: 200, body: {} }).as("deleteModel");
        ...
        cy.getBySel("confirmation-input").demoType(MODEL_NAME);
        cy.demoPause(300);
