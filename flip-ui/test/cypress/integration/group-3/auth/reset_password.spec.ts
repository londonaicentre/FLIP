/*
 * Copyright (c) 2026 Guy's and St Thomas' NHS Foundation Trust & King's College London
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *     http://www.apache.org/licenses/LICENSE-2.0
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */




const cognitoUrl = "https://cognito-idp.eu-west-2.amazonaws.com/";
const dummyEmail = "throwaway.user@kcl.ac.uk";

describe("Reset Password Page", () => {
    beforeEach(() => {
        cy.visit("/auth/change-password");
    });

    it("should be able to reset password", () => {
        cy.getBySel("email").clear().type(dummyEmail);
        cy.intercept("POST", cognitoUrl, { statusCode: 200, fixture: "auth/cognitoForgotPassword" }).as("requestCode");
        cy.getBySel("requestCode-btn").click();
        cy.intercept("POST", cognitoUrl, { statusCode: 200, fixture: "auth/cognitoConfirmForgotPassword" }).as("changePassword");
        cy.getBySel("password").clear().type("NewPassword!1");
        cy.getBySel("email").clear().type(dummyEmail).click();
        cy.getBySel("changePassword-btn").click();
        cy.contains("A confirmation code is required").should("be.visible");
        cy.getBySel("iNeedACode-btn").click();

        cy.getBySel("email").clear().type(dummyEmail);
        cy.intercept("POST", cognitoUrl, { statusCode: 400 }).as("requestCode");
        cy.getBySel("requestCode-btn").click();
        cy.contains("Something went wrong");

        cy.getBySel("email").clear().type(dummyEmail);
        cy.intercept("POST", cognitoUrl, { statusCode: 400 }).as("requestCode");
        cy.getBySel("iHaveACode-btn").click();
        cy.getBySel("confirmation-code").should("be.visible");
        cy.getBySel("iNeedACode-btn").click();
        cy.getBySel("requestCode-btn").should("be.visible");

        cy.getBySel("email").clear().type(dummyEmail);
        cy.intercept("POST", cognitoUrl, { statusCode: 200, fixture: "auth/cognitoForgotPassword" }).as("requestCode");
        cy.getBySel("requestCode-btn").click();
        cy.wait("@requestCode");
        cy.intercept("POST", cognitoUrl, { statusCode: 200, fixture: "auth/cognitoConfirmForgotPassword" }).as("changePassword");
        cy.getBySel("confirmation-code").clear().type("12345");
        cy.getBySel("password").clear().type("NewPassword!1");
        cy.getBySel("email").clear().type(dummyEmail);
        cy.getBySel("changePassword-btn").click();
        cy.get("@requestCode").its("request.body").should("deep.include", { "Username": dummyEmail });
        cy.get("@requestCode").its("request.body").should("have.property", "ClientId");
        cy.get("@changePassword").its("request.body").should("deep.include", {
            "Username": dummyEmail,
            "ConfirmationCode": "12345",
            "Password": "NewPassword!1"  // pragma: allowlist secret
        });
        cy.get("@changePassword").its("request.body").should("have.property", "ClientId");
    });
});
