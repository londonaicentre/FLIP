...
| Agent instructions (this file's `AGENTS`/`CLAUDE` pair, at any level) | Edit the `AGENTS`-named file — it is the canonical one, because `AGENTS.md` is the cross-tool standard and `CLAUDE.md` is one vendor's name for it. Beside each, `CLAUDE.md` is a relative git symlink (mode `120000`) pointing at it, so the pair cannot drift and there is no regeneration step; never replace a link with a copy. The direction costs nothing, because Claude Code's own loader follows the link: the `CLAUDE.md` name still resolves under every `instructionFiles` mode, and on Bedrock, Vertex and Foundry, where the `AGENTS.md` fallback does not exist at all. Writing through a link is refused rather than silently clobbering it, so an agent told to edit `CLAUDE.md` is sent back to the `AGENTS.md` beside it. Keep the two byte-identical for one further reason: a reader that loads both drops the duplicate by content, whereas the name-substituted copies this pair used to be differed, and so were loaded twice over in full. |
