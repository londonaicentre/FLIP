...
├── fl-tutorials/       # FL tutorials per backend (all NVFLARE ones are Client-API apps): fl-tutorials/nvflare/{image_*}, fl-tutorials/flower/{xray_classification,3d_spleen_segmentation*} (root Makefile forwards by FL_BACKEND); xray classification, spleen seg/eval, diffusion. Shared dataset tooling in fl-tutorials/datasets/ (download/derive/enrich, single copy for both backends — the download-*-data + upload-spleen-labels targets), outputs in the shared gitignored fl-tutorials/data/. fl-tutorials/datasets/utils/ holds the OMOP CDM contract (schemas + concept mappings) shared by the spleen OMOP-generation chain (`convert-spleen-to-dicom`, `create-spleen-metadata-table`, `build-spleen-omop-tables`, plus the reproducible-path/verification targets — #1092). Plus fl-tutorials/tests/ — CPU-only pytest over the tutorial transform chains (#871) plus a static `min_clients` wiring guard covering fl-apps/flower too, run by `make -C fl-tutorials test`
...
The spleen tutorials' mock OMOP data is generated by a vendored chain in `fl-tutorials/datasets/`
(FLIP#1092), reproducible without root via a pinned published metadata table and verified against
the published export:

```bash
make -C fl-tutorials fetch-spleen-metadata-table    # reproducible path: pinned metadata table
...
make -C fl-tutorials convert-spleen-to-dicom        # full regeneration path (workstation/root only)
make -C fl-tutorials create-spleen-metadata-table   # full regeneration path
```

See `fl-tutorials/datasets/README.md` ("Spleen OMOP mock-data generation") for the full chain,
including the `download-spleen-msd-raw` regeneration-path first step.
