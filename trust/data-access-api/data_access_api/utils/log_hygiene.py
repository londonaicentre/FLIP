...
def hash_query(query: object) -> str:
    ...
    ``services.query_cache._make_cache_key`` for a parameterless query, so cache
    log lines and error log lines for the same query carry the same fingerprint,
    and an operator holding the original SQL can re-derive it to find matching
    log lines.

    Args:
        query: The SQL query, as a string or anything whose ``str()`` is the SQL
            text (e.g. a SQLAlchemy ``TextClause``).
