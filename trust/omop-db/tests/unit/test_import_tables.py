...
class TestLoadProject:
    ...
    def test_remove_only_deletes_by_the_given_cuts_persons_and_loads_nothing(self, tmp_path, monkeypatch):
        """Unseeding a re-cut project: the OLD tables' person ids scope the delete; no insert follows (FLIP#1261)."""
...
class TestMain:
    ...
    def test_remove_only_unseeds_the_listed_projects_and_loads_nothing(self, tmp_path, monkeypatch, fake_engine):
        """--remove-only (FLIP#1261): remove_projects runs on the given tables; no validation of the full table
