...
| `deploy/compose_trust.{env}.gpu.yml` | GPU passthrough overlay — added by `up-trust` / `up-fl-clients-kit` via `GPU_OVERRIDE` only when the kit's `NUM_AVAILABLE_GPUS > 0`; reserves host NVIDIA GPU(s) for the fl-client. `up-trust-ec2` applies it only when `TRUST_EC2_NUM_GPUS > 0` (a GPU EC2 host on the Deep Learning Base GPU AMI); otherwise the fl-client is CPU-only there regardless of the kit's `NUM_AVAILABLE_GPUS`, which the template seeds to 1 |
