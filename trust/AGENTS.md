...
make check-governance KIT=GSTT   # Validate the trust's governance document — both halves, starts nothing
make reload-governance KIT=GSTT  # Re-apply an edited governance document to a LIVE trust: recreates data-access-api + the fl-clients only, touches no data
```
...
- Trust governance document (FLIP#1259): one optional TOML file per trust (`trust/governance.<CODE>.toml`, worked example `trust/governance.example.toml`) whose `[fl_privacy]` section is the same site policy above (it WINS over `FL_SITE_PRIVACY_*`, and the client logs which source it used) and whose `[disclosure]`/`[access]` sections state `COHORT_QUERY_THRESHOLD` (may raise, never lower) plus permit/deny rules for data-access-api. Named by `ACCESS_POLICY_FILE` in the kit as a **host** path relative to `trust/` — it is the bind-mount source; every service reads it at `/app/governance.toml`. `[fl_privacy]` cannot be stated inline in `ACCESS_POLICY`: the fl-client reads the file only, so `check-governance` refuses an inline document carrying it. Validate and apply to a live trust with `make -C trust check-governance` then `make -C trust reload-governance` — **never** `up-trust`/`restart-trust`, which are first-install verbs that can wipe XNAT and re-seed the data volumes. On Kubernetes the chart carries the same document as its `governance.document` value (`trust/deploy/helm/README.md`).
