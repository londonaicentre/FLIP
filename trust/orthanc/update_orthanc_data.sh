...
update_trust() {
  ...
  local local_archive="${VOLUMES_DIR}/trust${trust_num}_orthanc_data_${DATA_VERSION_SLUG}.tar"
  ...
  if [[ -f "${seed_marker}" && "${FORCE:-0}" != "1" ]]; then
    ...
    echo "   requested version is now ${DATA_VERSION}. Re-snapshotting discards every seeded study." >&2
