...
re-hosted (spleen since FLIP#1261) — is seeded with ``--source <dicom tree>
...
the PACS port and the credentials from the kit. Direct use::
    ...
    ORTHANC_USERNAME=… ORTHANC_PASSWORD=… uv run trust/orthanc/seed_orthanc.py \\
        --trust-index 1 --orthanc-url http://127.0.0.1:8042 --projects spleen_project \\
        --source ../../fl-tutorials/data/spleen/dicom --tables-dir ../../fl-tutorials/data/spleen/canonical
"""
...
def scan_local_tree(source: Path) -> dict[str, list[Path]]:
    ...
    A regenerated set is laid out however its converter chose — spleen's by subject, another
    dataset's by accession — so the tree is read the way ``publish_dicom.py`` reads it: by the tag,
    not the path.
    """
...
def seed_project(
    ...
    (spleen — FLIP#1261) — same selection by ``source_trust``, same zero-mismatch guard.
