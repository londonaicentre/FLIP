...
| AWS IMDS | `169.254.169.254/32` | EC2 metadata / IAM-role credentials. **The fl-client no longer needs this** — it holds no AWS credentials and its kit is staged onto the node beforehand. The one remaining consumer is the omop-db vocab-load Job, which reads the licensed vocabulary bundle from S3 (the trust-seed Job that replaced the OMOP/Orthanc snapshot init Jobs fetches the mock data anonymously from Hugging Face and carries no AWS env at all); on kind it reads file credentials via `omopDb.vocabLoad.hostAwsMount`, not IMDS. | IMDS is a known SSRF/cred-theft target — see hardening note. **NOT removable as shipped**: the chart has no IRSA path (no ServiceAccount carries an `eks.amazonaws.com/role-arn`), and the vocab-load Job gates its AWS env on `hostAwsMount.enabled`, which defaults to `false` — so by default it authenticates to S3 through the node instance role, i.e. IMDS. Removing this rule breaks the vocabulary load on any node-role-bearing cluster. Closing it means giving that Job an explicit credential path first. |
