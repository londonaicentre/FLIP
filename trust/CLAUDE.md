...
| `deploy/helm/` | The same trust stack as Helm chart `flip-trust` for Kubernetes (chart, Makefile, `sync_k8s_kit.py`, tests); moved here from `deploy/providers/kubernetes/` in #1213 because it renders the stack rather than provisioning infrastructure |
| `deploy/ansible/onprem.yml` | Ansible play that configures a site-owned Ubuntu host (Docker, `/opt/flip` dirs, uid rules) for the compose stack; on-prem twin of `deploy/providers/AWS/site.yml`, driven by `make -C deploy/providers/AWS provision-local-trust` |
