...
PAIRS=(
  ...
  # and model factory as byte-identical copies (the same preprocessing/architecture must train on
  # NVFLARE and Flower alike). The NVFLARE copy is the reference; resync by copying it over the
  # Flower copy. These pairs' reference side lives under fl-tutorials/nvflare/tabular_classification/,
  # which fl-apps-check-tutorial-sync.yml's path filters must include (see the NOTE below).
