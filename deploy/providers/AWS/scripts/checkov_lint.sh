...
# flagged resource/data block. The rationale is mandatory.
#
# Known limitation: the IAM checks only flag a literal "*" Resource/Action.
...
# rationale at the resource, so a new unjustified instance blocks the PR:
...
if ! grep -q "FAILED for resource" <<< "${canary_output}"; then
    echo "ERROR: canary fixture produced no findings — the checkov lint harness is broken." >&2
