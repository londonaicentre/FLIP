...
write_kit_file() {
    ...
    if [ "$has_creds" = true ]; then
        ...
        strip_re='^(FL_KIT_SLOT|FL_KIT_SLOT_NUMBER|'"${hub_keys_re}"')='
        strip_re="${strip_re}|${tail_re}"
    fi
...
register_one() {
    ...
    case "$host" in
        ...
        ec2)
            ...
            # file may not exist — scp will exit non-zero and we fall through
            # with an empty tempfile, which write_kit_file handles the same as
            # a fresh host (no source file).
            scp -q "flip-trust:/opt/flip/trust/.env.${slot}" "$tmp_kit" \
                2>/dev/null || : > "$tmp_kit"
