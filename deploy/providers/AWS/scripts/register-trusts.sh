...
write_kit_file() {
    ...
    # second `make up-trust` fails to bind. The ec2 path passes a fresh
    # tempfile so neither source exists and we fall through to credentials-only,
    # matching the previous behaviour for single-trust hosts.
    local source=""
    if [ -f "$path" ]; then
        source="$path"
    ...
    {
        if [ -n "$source" ]; then
            # Strip: generated-by comments, hub-shared keys, hub-shared header,
            # FL_KIT_SLOT / FL_KIT_SLOT_NUMBER (always re-emitted below).
            # On new-registration also strip the credential lines (they are about
            # to be replaced). On skip, leave them in place — we cannot re-emit them.
            local strip_re='^(FL_KIT_SLOT|FL_KIT_SLOT_NUMBER|'"$hub_keys_re"')=|^# Generated by register-trusts\.sh|^# Plaintext credentials — keep this file secret'
            if [ "$has_creds" = true ]; then
                strip_re='^(TRUST_API_KEY|TRUST_INTERNAL_SERVICE_KEY|EXPECTED_TRUST_ID|FL_KIT_SLOT|FL_KIT_SLOT_NUMBER|'"$hub_keys_re"')=|^# Generated by register-trusts\.sh|^# Plaintext credentials — keep this file secret'
            fi
