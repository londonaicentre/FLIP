...
write_kit_file() {
    ...
    {
        if [ -n "$source" ]; then
            grep -vE '^(TRUST_API_KEY|TRUST_INTERNAL_SERVICE_KEY|FL_KIT_SLOT|FL_KIT_SLOT_NUMBER|EXPECTED_TRUST_ID)=|^# Generated by register-trusts\.sh|^# Plaintext credentials — keep this file secret' "$source"
        fi
