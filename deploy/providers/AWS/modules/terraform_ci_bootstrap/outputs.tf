...
# change: instantiate this module in the new account, repoint the two variables.
...
# `var.iam_permissions_boundary_name`, rather than reading it from here — they
# share no state. Printed so a mismatch is visible without an AWS call:
...
output "plan_job_workflow_refs" {
  ...
  value       = local.plan_workflow_refs
}

output "state_bucket_arn" {
  description = "ARN of the state bucket when this module manages it, otherwise null."
