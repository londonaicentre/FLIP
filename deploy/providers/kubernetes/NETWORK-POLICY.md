...
| AWS IMDS | `169.254.169.254/32` | EC2 metadata / IAM-role credentials. **The fl-client no longer needs this** — it holds no AWS credentials and its kit is staged onto the node beforehand. Remaining consumers are cluster-dependent: on kind the OMOP/Orthanc seed Jobs read file credentials via `omopDb.initJob.hostAwsMount`, not IMDS. | IMDS is a known SSRF/cred-theft target — see hardening note. **NOT removable as shipped**: the chart has no IRSA path (no ServiceAccount carries an `eks.amazonaws.com/role-arn`), and both seed Jobs gate their AWS env on `hostAwsMount.enabled`, which defaults to `false` — so by default they authenticate to S3 through the node instance role, i.e. IMDS. Removing this rule breaks OMOP/Orthanc seeding on any node-role-bearing cluster. Closing it means giving the seed Jobs an explicit credential path first. |
