...
| AWS IMDS | `169.254.169.254/32` | EC2 metadata / IAM-role credentials. **The fl-client no longer needs this** — it holds no AWS credentials and its kit is staged onto the node beforehand. Remaining consumers are cluster-dependent: on kind the OMOP/Orthanc seed Jobs read file credentials via `omopDb.initJob.hostAwsMount`, not IMDS. | IMDS is a known SSRF/cred-theft target — see hardening note. **Candidate for removal**: verify nothing on the target cluster authenticates via the node role, then drop the rule. |
