...
def main(
    ...
    # ── Print AWS-side manual steps ──────────────────────────────────
    print()
    print("─" * 64)
    print("📋 AWS DEPLOYMENT MANUAL STEPS")
    print("─" * 64)
    print()
    print("The following steps must be performed on the AWS side to register")
    print(f"{trust_name} with the running staging/production system:")
    print()
    print(f"1. Update AWS Secrets Manager:")
    print(f"   Navigate to Secrets Manager → FLIP_API → Retrieve secret value")
    print(f"   → Edit → add the new hash to trust_api_key_hashes:")
    print(f"")
    print(f"     trust_api_key_hashes: {{..., \"{trust_name}\": \"{trust_api_key_hash}\"}}")
    print(f"")
    print(f"   Or update the Terraform variable TRUST_API_KEY_HASHES and run:")
    print(f"      cd deploy/providers/AWS")
    print(f"      terraform apply -target=module.flip_api_secret")
    print()
    print(f"2. Re-seed the hub database (to insert {trust_name} into the trusts table):")
    print(f"   The flip-api start-up seed reads TRUST_NAMES from the env.")
    print(f"   Redeploy flip-api so the seeding runs.")
    print(f"   For ECS: force a new deployment of the flip-api service.")
    print()
    print(f"3. Verify {trust_name} connectivity:")
    print(f"   Check the trust-api pod logs for successful heartbeats:")
    print(f"     kubectl -n flip-trust logs deploy/flip-trust-trust-api")
    print()
    print("─" * 64)
