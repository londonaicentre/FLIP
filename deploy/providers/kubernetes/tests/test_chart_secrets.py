...
def test_generated_secrets_file_is_not_tracked():
    ...
    tracked = subprocess.run(
        ["git", "ls-files", "--error-unmatch", "deploy/providers/kubernetes/values-secrets.yaml"],
        cwd=CHART_DIR.parents[2],
        capture_output=True,
        text=True,
    )
    assert tracked.returncode != 0, (
        "deploy/providers/kubernetes/values-secrets.yaml is tracked again. It is generated by "
        "scripts/generate_values.py and carries live trust credentials — keep it gitignored and "
        "edit values-secrets.yaml.example instead."
    )


def test_the_example_carries_no_populated_secret():
    """Every secret slot in the tracked example must be a placeholder.

    The example exists so the field list stays discoverable. It is the obvious
    place for someone to paste a real value "just to test", which is how the
    original file came to hold a live key.
    """
    ...
    # Structural or deliberately non-secret values. Anything else must look like
    # a placeholder: angle-bracketed, or an obvious weak default the chart
    # documents (the entrypoints reject those at runtime).
    allowed_literals = {
        "true",
        "false",
        "admin",
    ...
    for line in example.read_text().splitlines():
        match = re.match(r"^\s{4}([a-z0-9-]+):\s*(\S.*?)\s*$", line)
        if not match:
            continue
        value = match.group(2)
        if value in allowed_literals or value.strip('"').startswith("<"):
            continue
        offenders.append(match.group(1))

    assert not offenders, f"populated values in the tracked example: {offenders}"
