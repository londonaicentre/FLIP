...
- `AUTH_BACKEND` — `cognito` (AWS Cognito) or `keycloak` (the `keycloak` service in `deploy/compose.development.yml`, realm in `deploy/keycloak/flip-realm.json`) selects the identity provider (FLIP#919): both the `TokenRules` the generic OIDC verifier applies in `auth/token_verifier.py` (issuer, JWKS URL, audience or `client_id`, `token_use`/`typ`, principal-name claim) and the `IdentityProvider` implementation in `auth/identity/` that lists, invites, disables and deletes users and reads/clears MFA state — routers receive it through `Depends(get_identity_provider)`, the boot seed and scripts through `build_identity_provider()`, and the CORS allowlist is its `allowed_origins()` (Cognito callback URLs, Keycloak client redirect URIs). Same selection pattern as `EMAIL_BACKEND`: `DevSettings` defaults to `keycloak` (with working defaults for `KEYCLOAK_URL=http://keycloak:8080`, `KEYCLOAK_PUBLIC_URL=http://localhost:8180`, `KEYCLOAK_REALM=flip`, `KEYCLOAK_CLIENT_ID=flip-ui`, `KEYCLOAK_AUDIENCE=flip-api`, `KEYCLOAK_ADMIN_CLIENT_ID=flip-api-admin`, `KEYCLOAK_ADMIN_CLIENT_SECRET`), `ProdSettings` narrows the type to `Literal["cognito"]` so the dev provider is a boot-time `ValidationError` in stag/prod; the base `Settings` selects none, so the two `AWS_COGNITO_*` ids are optional there and required (an empty value counts as missing) whenever `AUTH_BACKEND=cognito` — always in stag/prod, only on request in dev; a deployment on another cloud adds `auth/identity/<name>.py` + a rules builder and widens `ProdSettings` deliberately — never an `if AUTH_BACKEND` in a router. The two Keycloak URLs differ on purpose: the public one is what the browser signs in through and therefore the `iss` every token carries (pinned by `KC_HOSTNAME` in the compose), the internal one is where flip-api fetches the JWKS and calls the Admin REST API (never OIDC discovery, which would advertise the unreachable public `jwks_uri`). Errors leave every provider as the neutral exceptions in `auth/identity/errors.py` and become HTTP status codes in exactly one place (`auth/identity/http.py`: 404/400/503/500), which the routers' existing `except HTTPException` branches rely on. Under keycloak a user registered from the Admin Area is given `ADMIN_USER_PASSWORD` as a temporary password when the realm has no SMTP (dev only, `ENV == development`; the log names the variable, never the value — CodeQL flags clear-text password logging), and `ENFORCE_MFA=true` only logs a warning: the hub gate works off OTP credentials, but the UI's password grant cannot enrol or answer TOTP. The provider is also what `tests/integration/utils.py::admin_authentication` (e2e smoke, `create_testing_projects`, demo tooling) signs in through — Keycloak's password grant against the public URL, or the IAM-gated Cognito flow.
