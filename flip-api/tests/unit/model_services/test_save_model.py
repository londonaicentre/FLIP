...
@pytest.fixture
def mock_can_contribute_to_project_true():
    with patch("flip_api.model_services.save_model.can_contribute_to_project", return_value=True):
        yield


@pytest.fixture
def mock_can_contribute_to_project_false():
    with patch("flip_api.model_services.save_model.can_contribute_to_project", return_value=False):
        yield
...
def test_save_model_success(
    ...
    mock_can_contribute_to_project_true,
...
def test_save_model_forbidden(mock_can_contribute_to_project_false):
    response = client.post("/api/model", json=test_payload)
    assert response.status_code == status.HTTP_403_FORBIDDEN
    assert "is not allowed" in response.json()["detail"]


def test_save_model_project_not_found(mock_can_contribute_to_project_true, mock_get_project_not_found):
    response = client.post("/api/model", json=test_payload)
    assert response.status_code == status.HTTP_404_NOT_FOUND
    assert "No project found" in response.json()["detail"]


def test_save_model_project_not_approved(mock_can_contribute_to_project_true, mock_get_project_not_approved):
    response = client.post("/api/model", json=test_payload)
    assert response.status_code == status.HTTP_400_BAD_REQUEST
    assert "is not approved" in response.json()["detail"]


def test_save_model_no_approved_trusts(
    mock_can_contribute_to_project_true,
    mock_get_project_approved,
    override_dependencies,
):
...
def test_save_model_database_error(
    mock_can_contribute_to_project_true,
    mock_get_project_approved,
    override_dependencies,
):
