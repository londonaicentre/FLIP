...
signatures; here ``verify_access_token`` is stubbed so each test isolates the
gate's own decisions.
"""
...
PATCH_MFA = "flip_api.auth.dependencies.is_mfa_enabled"
...
def test_verify_token_allows_mfa_enrolled_caller(credentials, user_sub):
    ...
    with patch(PATCH_VERIFY, return_value=_identity(user_sub)), patch(PATCH_MFA, return_value=True) as mock_mfa:
    ...
    assert mock_mfa.call_args.args[0] == "user@example.com"
...
def test_verify_token_rejects_caller_without_mfa(credentials, user_sub):
    ...
    with patch(PATCH_VERIFY, return_value=_identity(user_sub)), patch(PATCH_MFA, return_value=False):
...
def test_verify_token_propagates_verification_failures(credentials):
    ...
    with patch(PATCH_VERIFY, side_effect=rejected), patch(PATCH_MFA) as mock_mfa:
        with pytest.raises(HTTPException) as exc_info:
            verify_token(credentials)

    assert exc_info.value.status_code == status.HTTP_401_UNAUTHORIZED
    mock_mfa.assert_not_called()
...
def test_verify_token_no_mfa_skips_gate(credentials, user_sub):
    ...
    with patch(PATCH_VERIFY, return_value=_identity(user_sub)), patch(PATCH_MFA) as mock_mfa:
...
def test_verify_token_skips_mfa_gate_when_enforce_mfa_is_false(credentials, user_sub):
    ...
    with (
        patch(PATCH_VERIFY, return_value=_identity(user_sub)),
        patch(PATCH_MFA) as mock_mfa,
        patch(PATCH_SETTINGS) as mock_get_settings,
    ):
...
def test_verify_token_enforces_gate_when_enforce_mfa_is_true(credentials, user_sub):
    ...
    with (
        patch(PATCH_VERIFY, return_value=_identity(user_sub)),
        patch(PATCH_MFA, return_value=False) as mock_mfa,
        patch(PATCH_SETTINGS) as mock_get_settings,
    ):
