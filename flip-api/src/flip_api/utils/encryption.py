...
- ``AES_KEY_BASE64`` / Secrets Manager ``aes_key`` — the shared key, always
  registered under :data:`SHARED_KID`. Used when nothing else is configured.
...
def _shared_key() -> bytes:
    """Return the shared AES key (Secrets Manager in prod, settings in dev)."""
    stt = get_settings()
    key_b64 = get_secret("aes_key") if stt.ENV == "production" else stt.AES_KEY_BASE64
...
def kid_for_trust(trust_id: str | None = None, trust_code: str | None = None) -> str:
    """Return the ``kid`` to encrypt with for a given trust.

    Per-trust keys are registered as ``trust-<trust_id>``. Falls back to the
    default kid when that trust has no key yet, so a trust can be given its own
    key independently of the others.

    Args:
        trust_id (str | None): The trust's UUID (preferred, stable).
        trust_code (str | None): The trust's short code (fallback label).

    Returns:
        str: A ``kid`` present in the keyring.
    """
    ring = _keyring()
    for candidate in (f"trust-{trust_id}" if trust_id else None, f"trust-{trust_code}" if trust_code else None):
        if candidate and candidate in ring:
            return candidate
    return _default_kid()
