...
def can_access_project(user_id: UUID, project_id: UUID, db: Session) -> bool:
    ...
    # No HTTPException re-raise: an access check returns a decision, not a message, and
    # must fail closed on any error — the handler below is deliberately the only exit.
