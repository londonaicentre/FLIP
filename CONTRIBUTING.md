...
| `MIN_CLIENTS=1` is back in `.env.development.example` (FLIP#1230) | Add `MIN_CLIENTS=1` to your `.env.development` if it is missing — the pre-commit hook will ask for it. The composes now default it to `1` when unset, so an existing stack keeps working either way; a checkout without the line only ever broke fresh clones, where compose rendered it as an empty string and every NVFLARE job failed at start. |
