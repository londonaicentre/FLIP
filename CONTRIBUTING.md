...
The chart has a `check_status.py` smoke test script (which also compares the running xnat-web pod's plugin jars against `xnat.web.plugins.urls`), a `scripts/smoke-cstore.sh` DICOM ingest smoke, and a `register_k8s_trust.py` registration script. See the [K8s README](trust/deploy/helm/README.md) for details.
