...
"""MONAI Deploy application template for FLIP segmentation models (DICOM SEG output).

Derived from ``ai_spleen_seg_app`` in the MONAI Deploy App SDK examples, retained under the
Apache 2.0 licence with the MONAI Consortium's copyright intact.

The pipeline is model-agnostic: it consumes a bundle-style TorchScript ``model.ts`` whose embedded
``inference.json`` supplies the preprocessing and inferer, so adapting this template to a different
segmentation model means changing the bundle and the two application-specific blocks marked below
(``segment_descriptions`` and ``Sample_Rules_Text``), not the operator graph.
"""
