...
3. Each site runs the user's `evaluator.py` via `ClientAPIExecutor` (in-process mode), using the NVFLARE Client
...
- `validate` → `nvflare.app_common.executors.ClientAPIExecutor` (in-process mode)
