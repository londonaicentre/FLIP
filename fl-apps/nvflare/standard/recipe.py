...
"""Driver script: regenerate the committed standard app/config JSONs and meta.json.

Run from the flip-utils venv (needs the ``full`` extra — flip + nvflare + torch):

    cd flip-utils && uv run --no-sync python \\
        ../fl-apps/nvflare/standard/recipe.py \\
        --output ../fl-apps/nvflare/standard
...
def main() -> None:
    """Export FlipFedAvgRecipe configs into the standard template directory."""
    parser = argparse.ArgumentParser(
        description="Regenerate the standard app/config JSONs and meta.json from FlipFedAvgRecipe."
    )
    parser.add_argument(
        "--output",
        type=Path,
