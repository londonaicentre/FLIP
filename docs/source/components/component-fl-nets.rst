...
The `standard` examples show how one job type runs quite different user-uploaded applications: each
performs a federated averaging training, but the data, architecture and training configuration
differ. The EHR one goes furthest — its cohort is tabular OMOP data with no imaging in it at all,
which is possible because a job type describes the NVFLARE job FLIP assembles around your files,
never the modality. What to fetch is the uploaded script's decision: an imaging app calls
``flip.get_by_accession_number``, the EHR one calls only ``flip.get_dataframe``.
