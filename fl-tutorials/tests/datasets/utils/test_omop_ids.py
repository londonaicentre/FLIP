...
def test_every_known_project_has_a_distinct_block(omop_ids: ModuleType) -> None:
    """cxr, spleen, prostate and brain_mri must each get their own non-overlapping block.

    4_000_000 is deliberately absent: pathology_project (FLIP#1181) takes it, so brain_mri skipped to
    5_000_000 rather than collide with it in a shared trust database.
    """
    ...
    assert 4_000_000 not in blocks.values()
