...
CPU-only pytest suite over the tutorial apps: their transform chains, a static drift guard on the
Flower apps' `min_clients` wiring (which also covers `fl-apps/flower/`, the templates that actually
deploy), and the EHR risk-prediction tutorial's shared feature engineering + model contract
(`test_ehr_feature_engineering.py` — that app reads no DICOM, so it is deliberately absent from
`DICOM_APPS`). No GPU, no dataset download, no FL image, no network — the fixtures are synthetic
DICOMs and dataframes built in-process, and the whole suite runs in a couple of seconds.
