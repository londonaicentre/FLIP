...
# space and construct no PerceptualLoss. It moved with the split of the old two-stage latent
# diffusion job into three single-stage ones — this file's `test_ldm_offline_backbone.py` ancestor
# guarded the same contract when the autoencoder phase was part of that job.
...
def trainer():
    # trainer.py imports its flat siblings (models, transforms, debug_samples) by bare name, as the
    # deployed job does; put the app dir first on the path for the import, then drop the bare
    # names it registered so no other app's same-named module resolves to the autoencoder copy.
    before = set(sys.modules)
    sys.path.insert(0, str(APP_DIR))
    try:
        module = load_module("autoencoder_trainer_under_test", APP_DIR / "trainer.py")
    finally:
        sys.path.remove(str(APP_DIR))
        for name in set(sys.modules) - before - {"autoencoder_trainer_under_test"}:
            if "." not in name:
