...
"""Pixel-space diffusion FedAvg Client API job definition for FLIP.

Trains a DiffusionModelUNet that denoises images directly, as an ordinary single-stage
FedAvg job, using :class:`FlipFedAvgRecipe`. It supports two execution modes:

*Export* (``--export --export-dir <path>``):
    Writes the complete NVFLARE job to ``<path>/flip_fedavg/``, including ``meta.json``
    (carrying ``custom_props.model_id`` for lazy resolution), ``app/config/`` (server + client
    configs), and ``app/custom/`` (the bundled ``flip/`` package plus the staged user app files).
    This is the primary, fully-local-verifiable path — no GPU or data required.

*SimEnv* (default, no flags):
    Runs a local simulation under the NVFLARE simulator. Requires a GPU and the reference dataset
    (``make -C fl-tutorials download-xray-data``). FLIP-specific values are injected via
...
def stage_app_files(job: FedJob) -> None:
    ...
    ``models.py``, ``config.json``, ``transforms.py``) must be added to the job explicitly so they
...
def main() -> None:
    parser = argparse.ArgumentParser(description="FLIP Pixel-space Diffusion Client API FedAvg Job")
    ...
    parser.add_argument(
        "--workspace",
        type=str,
        default="/tmp/nvflare/diffusion_model",
