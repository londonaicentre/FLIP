...
"""FedAvg Client API job definition for EHR risk prediction with FLIP.
...
*SimEnv* (default, no flags):
    Runs a local simulation under the NVFLARE simulator. CPU is enough for this
    tabular MLP; download the reference dataset first
    (``make -C fl-tutorials download-synthea-data``). FLIP-specific values are
    injected via environment variables (``FLIP_PROJECT_ID``, ``FLIP_QUERY``)
    rather than CLI flags because SQL queries contain spaces that don't survive
    argparse whitespace-splitting.
...
Usage:
    ...
    # SimEnv local simulation (CPU is enough; needs the Synthea data)
...
def stage_app_files(job: FedJob) -> None:
    ...
    ``models.py``, ``config.json``, ``feature_engineering.py``) must be added to the job
...
def main() -> None:
    parser = argparse.ArgumentParser(description="FLIP EHR Risk Prediction Client API FedAvg Job")
