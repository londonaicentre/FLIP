...
# Ark+ Fine-tuning — Chest X-ray Classification (NVFLARE Client API variant)

FLIP tutorial for **federated fine-tuning** of an Ark+ Swin foundation model on chest X-ray
classification, using the NVFLARE **Client API** (`nvflare.client`) rather than the legacy Executor
API. It is the sibling of [`arkplus_fine_tuning`](../arkplus_fine_tuning/README.md): the finetuning
semantics are identical (frozen backbone, fresh 5-class head, local teacher/student EMA), only the FL
plumbing differs. The job is defined entirely in Python via
...
make -C fl-tutorials run-tutorial TUTORIAL=arkplus_fine_tuning_client_api
