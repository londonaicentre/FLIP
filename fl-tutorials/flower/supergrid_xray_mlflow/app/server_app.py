...
"""SuperGrid x MLflow spike ServerApp (FLIP#744 x FLIP#745).

The FLIP#744-spike-proven xray ServerApp, extended to write metrics AND the
trained model weights to an MLflow tracking server configured entirely via
Flower run-config — because on SuperGrid this process runs in a Flower-managed
pod with no FLIP environment, no /opt/flip-utils, and no artifact download
path (`flwr pull` is roadmapped; Flower Labs' recommendation is exactly this:
ship artifacts out via an experiment tracker).
"""
...
from app.mlflow_logging import setup_mlflow
...
def main(grid: Grid, context: Context, flip: FLIP = FLIP()) -> None:
    ...
    model_id = run_config.get("flip-model-id", "supergrid-xray-mlflow-model")
    num_rounds = int(run_config.get("num-server-rounds", 1))

    # MLflow first: its probe log lines are the earliest signal of whether the
    # pod can reach the tracker at all (failures downstream are best-effort).
    mlflow_logger = setup_mlflow(run_config, model_id)
    ...
    # Use FedAvg strategy with per-client metrics tracking + live MLflow mirror
    ...
    # The spike's raison d'être: weights + results leave the pod via MLflow.
    if mlflow_logger is not None:
        mlflow_logger.log_final_model(output_dir, registered_name=f"flip-supergrid-{model_id}")
