...
is generated in-tree, per dataset. Two projects are covered: `spleen_project` (the whole chain,
from a public MSD download; its DICOM set regenerates locally and is never published) and
`cxr_project` (the OMOP conversion only; see below for why). Both share one contract in
[`utils/`](utils/) and one verification gate.

### Where the DICOMs live (FLIP#1261)

MSD Task09_Spleen is open data, so FLIP does not re-host DICOMs cut from it. What
`aicentreflip/trust-data` carries for that project is **the OMOP tables and the metadata table they
were built from** (`omop-csv/<project>/`, `omop-csv/<project>/source/`); the
...
2. **NIfTI → DICOM** (`convert-spleen-to-dicom`), one CT series per subject under
   ...
   spleen DICOMs had to be re-hosted before FLIP#1261.
...
`build-spleen-omop-tables` → `verify-spleen-omop-tables` in one command. Since the FLIP#1261 cut
...
**Moving a trust that holds the pre-FLIP#1261 spleen cut.** Every spleen identity, accession and
...
seeds by default are the ones that publish a DICOM set — spleen is no longer among them.
...
- `omop_ids.py` — the per-project surrogate-key blocks (`cxr_project` 1M, `spleen_project` 2M,
  `prostate_project` 3M; 4M is `pathology_project`'s, FLIP#1181). All
...
- `dicom_writer.py` — the deterministic NIfTI → DICOM series writer (pydicom + nibabel) a dataset's
  converter builds on: RAS affine → LPS geometry, MR pixels unsigned / CT as signed HU, every UID
  from `generate_uid(entropy_srcs=…)` over the caller's stable identifiers, no wall clock.
...
Those are properties of the dataset — spleen *mocks* `Manufacturer` and `InstitutionName` from the
shared tables; prostate (FLIP#1091) carries real ones recovered from
