...
is generated in-tree, per dataset. Three projects are covered: `spleen_project` and
`brain_mri_project` (the whole chain, from a public MSD download; their DICOM sets regenerate
locally and are never published) and `cxr_project` (the OMOP conversion only; see below for why).
