...
```bash
docker exec -it deploy-fl-api-net-1-1 flwr log <run-id> local --show       # hub dev stack (net 1)
docker compose -f fl-services/flower/compose.dev.yml exec fl-api \
    flwr log <run-id> local --show                                         # standalone dev stack
```
...
still going it follows the log until then. Run ids come from `flwr list local` in the same
