...
The project supports [NVIDIA FLARE](https://developer.nvidia.com/flare) and [Flower](https://flower.ai/). Each backend
provisions its own credentials in-tree (gitignored) under `fl-services/<backend>/provision/`, and its own Makefile owns
provisioning:

- **NVFLARE** — per-participant "startup kits" (root-CA-signed certs + configs) under `provision/workspace-{dev,stag,prod}/`.
  See [`fl-services/nvflare/README.md`](fl-services/nvflare/README.md).
- **Flower** — per-net TLS certs + SuperNode auth keys under `provision/creds/`.
  See [`fl-services/flower/README.md`](fl-services/flower/README.md).

> **Provision _before_ `make up`.** Credentials are **not** generated by `make up` — provision first, or the FL
> containers crash-loop (NVFLARE: `start.sh: No such file or directory`). From the repo root:
>
> ```bash
> make -C fl-services/nvflare provision-2-nets        # NVFLARE dev: net-1 + net-2 → provision/workspace-dev/
> make -C fl-services/flower  provision NET_NUMBER=1  # Flower: per-net certs + keys → provision/creds/net-1/
> make -C fl-services/flower  provision NET_NUMBER=2
> ```
>
> Then bring the stack up (`make up`, or `make up FL_BACKEND=flower`). Provisioning _after_ a `make up` is awkward:
> Docker will already have created the workspace `net-*` dirs as **root-owned** bind-mount targets, which block
> re-provisioning's `rm -rf` until they are removed.

**Dev vs stag/prod.** Dev provisions a fixed 2 clients each run. Stag/prod instead **over-provision** spare client
slots — `make -C fl-services/nvflare provision-stag` / `provision-prod` mint `Trust_1 .. Trust_N` (default 50 / 500) so
onboarding a new trust just claims a pre-minted `Trust_N` kit at `register_trust`, with no fleet-wide re-provision or
cert churn (see [FLIP#626](https://github.com/londonaicentre/FLIP/issues/626)). The stag/prod kits are uploaded to S3
with `make -C fl-services/nvflare upload-kits-to-s3 PROD=stag|true`.

**Path resolution.** `FL_PROVISIONED_DIR` is derived from `FL_BACKEND` in [`deploy/fl_backend.mk`](deploy/fl_backend.mk)
— `fl-services/nvflare/provision/workspace-dev` (nvflare) or `fl-services/flower/provision/creds` (flower) — and
absolutised by the Makefile so Docker volume mounts resolve. Override for a one-off with `make up FL_PROVISIONED_DIR=/tmp/ws`.

If you see `fed_client.json does not exist`, `missing startup folder`, or `start.sh: No such file or directory` in the
`fl-server` logs, the credentials weren't provisioned before `make up` — provision (above), then `make up`.
