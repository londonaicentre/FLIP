...
[`fl-services/nvflare/workspace-dev`](fl-services/nvflare/workspace-dev) (`make -C fl-services/nvflare provision`) and the Flower certs
at [`fl-services/flower/certs`](fl-services/flower/certs) (`make -C fl-services/flower provision`) — see
...
federated learning. Both require provisioned certificates and configuration files — for NVFLARE the per-participant
"startup kits", for Flower the per-net TLS certs + SuperNode keys — that are generated in-tree (gitignored) under
`fl-services/<backend>/`: NVFLARE at `fl-services/nvflare/workspace-dev/`, Flower at `fl-services/flower/certs/`.

> **Provision _before_ `make up` (NVFLARE).** The startup kits are **not** generated by `make up`. Provision them
> first, or the `fl-server` containers crash-loop on `start.sh: No such file or directory`. From the repo root:
>
> ```bash
> make -C fl-services/nvflare provision-2-nets    # dev: provisions net-1 and net-2 into fl-services/nvflare/workspace-dev/
> # or, for a single network: make -C fl-services/nvflare provision NET_NUMBER=<N>
> ```
>
> Then bring the stack up with `make up`. Provisioning _after_ a `make up` is awkward: Docker will already have
> created `fl-services/nvflare/workspace-dev/net-*` as **root-owned** bind-mount targets, which block
> re-provisioning's `rm -rf` until they are removed. See [`fl-services/nvflare/README.md`](fl-services/nvflare/README.md)
> for per-network details and how to onboard additional clients.
>
> For Flower, provision the per-net certs + SuperNode keys with `make -C fl-services/flower provision NET_NUMBER=<N>`
> (net-1 and net-2 for the default dev stack) before `make up FL_BACKEND=flower`.
...
1. **Path Resolution**: `FL_PROVISIONED_DIR` is derived from the `FL_BACKEND` selection inside [`deploy/fl_backend.mk`](deploy/fl_backend.mk):
   ...
   - `FL_BACKEND=nvflare` → `fl-services/nvflare/workspace-dev` (in-tree)
   - `FL_BACKEND=flower` → `fl-services/flower/certs` (in-tree)
...
- The workspace was provisioned with NVFLARE startup kits (`make -C fl-services/nvflare provision-2-nets`) **before** `make up`
- The `FL_PROVISIONED_DIR` path is correctly resolved (check Makefile output)
- For NVFLARE: the workspace is at `fl-services/nvflare/workspace-dev/`
