...
`fl-services/<backend>/`: NVFLARE at `fl-services/nvflare/provision/workspace-dev/`, Flower at `fl-services/flower/provision/creds/`.
...
>
> ```bash
> make -C fl-services/nvflare provision-2-nets    # dev: provisions net-1 and net-2 into fl-services/nvflare/provision/workspace-dev/
...
> created `fl-services/nvflare/provision/workspace-dev/net-*` as **root-owned** bind-mount targets, which block
...
1. **Path Resolution**: `FL_PROVISIONED_DIR` is derived from the `FL_BACKEND` selection inside [`deploy/fl_backend.mk`](deploy/fl_backend.mk):
   ...
   - `FL_BACKEND=nvflare` → `fl-services/nvflare/provision/workspace-dev` (in-tree)
   - `FL_BACKEND=flower` → `fl-services/flower/provision/creds` (in-tree)
...
- For NVFLARE: the workspace is at `fl-services/nvflare/provision/workspace-dev/`
