...
`deploy/providers/<backend>/`: NVFLARE at `deploy/providers/nvflare/workspace/`, Flower at `deploy/providers/flower/certs/`.
...
> created `deploy/providers/nvflare/workspace/net-*` as **root-owned** bind-mount targets, which block
...
1. **Path Resolution**: `FL_PROVISIONED_DIR` is derived from the `FL_BACKEND` selection inside [`deploy/fl_backend.mk`](deploy/fl_backend.mk):
   ...
   - `FL_BACKEND=nvflare` → `deploy/providers/nvflare/workspace` (in-tree)
   - `FL_BACKEND=flower` → `deploy/providers/flower/certs` (in-tree)
